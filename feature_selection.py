...
def feature_selection(x_train, y_train, x_val, y_val, categories, weights):
    ...
    def get_optimal_k_rfecv(X, Y, num_classes):
        # Use ROC-AUC for binary, Weighted F1 for multiclass
    ...
    x_val = x_val[x_train.columns] # Sync validation columns

    # These features are ALWAYS used as per your requirement
    ...
    # Track features that MUST be in the final set
    forced_features = [c for c in x_train.columns if 'Patient_Phenotype' in c]
    expert_opinion_features = [] # Placeholder for future manual additions
    ...
    for cat in categories:
        ...
        # Filter for columns actually present after cleanup
        ...
        # Rank features within this category to pick the best ones
    ...
    # --- 3. Interaction Term Injection ---
    # We combine forced features and category winners to find top interaction candidates
    current_set = list(set(selected_from_categories + forced_features))
    
    # Find top 5 strongest signals overall for interactions
    overall_scores, _ = f_classif(x_train[current_set], y_train)
    top_interact_names = pd.Series(overall_scores, index=current_set).sort_values(ascending=False).head(5).index
    
    interaction_cols = []
    for a, b in combinations(top_interact_names, 2):
        new_col = f"{a}_X_{b}"
        x_train[new_col] = x_train[a] * x_train[b]
        x_val[new_col] = x_val[a] * x_val[b]
        interaction_cols.append(new_col)
    
    # Final Selection: Categories + Clusters + Expert + Interactions
    final_feature_list = list(set(current_set + interaction_cols))
